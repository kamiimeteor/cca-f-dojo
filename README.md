...
| | 中文 | 交互式刷题 | 官方规格模考 | 题↔笔记跳转 |
| --- | :---: | :---: | :---: | :---: |
| [cca-f-complete-guide-cn](https://github.com/cyrus-tt/cca-f-complete-guide-cn)（10★） | ✅ | ❌ | ❌ | ❌ |
| [cca-f-study-zh](https://github.com/tigerbojo/cca-f-study-zh) | ✅ 繁体 | ❌ | ❌ | ❌ |
...
**中文的那几个是「读物」，能练的那几个不支持中文。** 本项目要占的就是这个交集。
...
暂未见同类实现。它和「程度判断」都是**可选的**，放在刷题页单独一块，不影响常规路径。
...
页脚可以**导出 / 导入 / 清空** —— 换设备或换浏览器时用导出的 JSON 迁移。

导入时会走 `sanitizeState()`：只保留已知字段、题目 id 与小节 id 走白名单、
