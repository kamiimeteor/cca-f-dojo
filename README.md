...
| 时长 | 120 分钟 |
| 题型 | **单选 + 多选混合**，每题写明要选几项 |
| 结构 | 优先从随机抽中的 4 个场景出题；某个 Domain 题量不足时，从其余场景补齐 |
...
**可选的云同步**：想换设备接着刷，可以用邮箱收一次性验证码登录，进度会存到云端。
进度与邮箱存在 Supabase 的 **AWS 伦敦区（eu-west-2）**；登录邮件由 Resend 从**爱尔兰（EEA）**投递，
内容仅为一次性验证码，不含任何学习数据。行级安全策略保证**每个账号只能读写自己那一行**，
...
{ id:'q164', d:'d1', s:'1.1', sc:'cs', diff:2,
  ...
  a:1,                                    // 正确项下标
  e:'解析…',
  w:{ 0:'A 为什么错', 2:'C 为什么错' },   // 每个错误项都要有，见下「逐项解析」
  near:0 }                                // 最强干扰项下标，见下
```
...
`near` 指向**最说得通、只是不够相称**的那个错误项，「程度判断」拿它当对手。
不标的话该模式会在所有干扰项里随机挑，容易挑到一眼假的选项，题就白出了。
约束：`near ≠ a`、下标在范围内、且 `w[near]` 要有解析。
...
答错后会展开一张表，逐个说明每个选项。所以 `w` 里**每个错误选项都要有一条**，
